...
queue <int> Q; deque <int> Dq; stack <int> st;
string s;
bool DEBUG = 1, avail[N];

ll m, n, c[N][N], res = -oo, start;
...
void Attempt(ll i, ll omax, ll omin, ll cnt)
{
    up(j , 1 , n) {
        if(avail[j] == 0 && c[i][j] > -1) {
            avail[j] = 1;
            Attempt(j , max(omax, c[i][j]) , min(omin, c[i][j]) , cnt + 1);
            avail[j] = 0;
        }
    }

    if(cnt > 2 && c[i][start] > -1) {                          /// cnt : so dinh da di; >= 3 la co kha nang quay lai dc
        omax = max(omax , c[i][start]);
        omin = min(omin , c[i][start]);
        res = max(omax + omin, res);
    }
}
